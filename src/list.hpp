...
#include <sstream>
#include "nodes/node_dll.hpp"
#include "nodes/node_dl.hpp"
...
namespace stc {
    ...
    public:
        ...
        ListLL(std::initializer_list<T> list) : ListLL() {
            for (auto i: list) {
                push_back(i);
            }
        }
    ...
    template<typename T>
    void ListLL<T>::push(T data, uint32_t i) {
        if (i < len() + 1) {
            if (i == 0) {
                push_front(data);
                return;
